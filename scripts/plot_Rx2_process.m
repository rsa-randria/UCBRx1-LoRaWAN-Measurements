...
%% Definition du dossier principal
base_dir = '../data/raw/power_profiling/receive_windows/rx2';

% Vérifier que le dossier existe
if ~isfolder(base_dir)
    error('ERREUR : Le dossier "%s" n''existe pas. Vérifie le chemin !', base_dir);
end

%% Liste des fichiers CSV et des labels correspondants
files = { 
    'ppk-rx2-standard-NACK.csv',...
    'ppk-rx2-standard-ACK.csv',...
    'ppk-rx2-ucb-ACK.csv'
};

labels = {'standard no ACK', 'standard with ACK','with UCB with ACK'};

%% Création de la figure pour le tracé global
fig = figure('Units', 'inches', 'Position', [200, 100, 10, 12]);
set(fig, 'Color', 'white');                          
set(fig, 'PaperUnits', 'inches', 'PaperPosition', [200, 100, 10, 12]);
...
for i = 1:length(files)
    ...
    chemin_fichier = fullfile(base_dir, files{i});
    ...
    end_idx = min(length(Current_mA), start_idx + 20000 - 1);
...
plot_path_fig = fullfile(output_folder, 'rx2_compare.fig');
plot_path_eps = fullfile(output_folder, 'rx2_compare.eps');
