...
static const uint8_t LoRaMacMaxEirpTable[] = { 8, 10, 12, 13, 14, 16, 18, 20, 21, 24, 26, 27, 29, 30, 33, 36 };
 ...
 /* ========================================== */
 static bool ucbActive ;
 static uint8_t ucbChannelValue = 0;
 /* ========================================== */
...
void LoRaMac_ucbChannel(bool ucb, uint8_t channel)
{
	ucbActive = ucb;
    ucbChannelValue = channel;
}
...
/*
// Vérifier si le bit ACK est positionné (bit 5 du FCtrl, valeur 0x20)
uint8_t fctrl = payload[5];  // Exemple : index 5, selon l'organisation (MHDR 1 octet, DevAddr 4 octets)
if( (fctrl & 0x20) != 0 )
{
	// L'ACK a été reçu
	MacCtx.McpsConfirm.AckReceived = true;

	// Tester dans quelle fenêtre l'ACK a été reçu
	if( MacCtx.RxSlot == RX_SLOT_WIN_1 )
	{
		// ACK reçu sur la fenêtre Rx1
		//MW_LOG(TS_OFF, VLEVEL_M, "ACK reçu sur Rx1\r\n");
	}
	else if( MacCtx.RxSlot == RX_SLOT_WIN_2 )
	{
		// ACK reçu sur la fenêtre Rx2
		//MW_LOG(TS_OFF, VLEVEL_M, "ACK reçu sur Rx2\r\n");
	}
}
*/
}
...
static LoRaMacStatus_t ScheduleTx( bool allowDelayedTx )
{
    ...
    if(ucbActive)
    {
    	MacCtx.Channel = ucbChannelValue;
    	MW_LOG(TS_ON, VLEVEL_M, "==> Canal UCB par l'application : %d\r\n", MacCtx.Channel);
